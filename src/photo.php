    ...
    <div class="box">
        ...
        <div class="container">
            ...
            function getPictures(){
                ...
                $row_user = $res->fetch();
                $uid = $row_user['UID'];
                $sql_id = "select ImageID,PATH,Title,Description from travelimage where UID='$uid'";
                $result = $pdo-> query($sql_id);
                if ($result->rowCount()>0) {
                    $num = 0;
                    $pictures = [];
                    while ($row = $result->fetch()) {
                        $pictures[$num] = $row['ImageID'];
                        $num++;
                    }
                    ...
                    $count = 0;
                    for ($i=3*$curpage - 3;$i < count($pictures);$i++){
                        $sql = "select * from travelimage where ImageID={$pictures[$i]}";
                        $result2 = $pdo->query($sql);
                        if ($row2 = $result2->fetch()) {
                            outputSingleGenre($row2);
                        }
                        $count++;
                        if ($count == 3){
                            break;
            ...
            function outputSingleGenre($row) {
                ...
                echo '<a href="./php/upload_revise.php?action=revise&id='.$row['ImageID'].'"><button>修改</button></a>';
                echo '<a href="?action=delete&id='.$row['ImageID'].'"><button>删除</button></a>';
                echo "</a>";
            ...
            function constructGenreLink($id) {
                if (isset($_SESSION['UserName'])) {
                    echo '<a href="./details.php?id=' . $id . '" class="details">';
                }else{
...
alert("请先登录！");
            ...
            if (isset($_GET['action'])&&$_GET['action']=="delete"){
                ...
                $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $sql_search = "select * from travelimage where ImageID={$imageID}";
                $res = $pdo->query($sql_search);
                $row = $res->fetch();
                $path = "../images/travel-images/medium";
                $filename = $row['PATH'];
                if (file_exists($path)){
                    $res = unlink($path.'/'.$filename);
                }
                $sql_delete = "Delete from travelimage where ImageID={$imageID}";
                $res2 = $pdo->query($sql_delete); //删除
                $pdo=null;
                getPictures();
            }else {
                getPictures();
