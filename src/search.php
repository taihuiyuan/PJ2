...
require_once ('./php/search_search.php');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<script>
    var radios1 = document.getElementById("search1");
    var radios2 = document.getElementById("search2");
    var searchContent;
    ...
    function showResults() {
        ...
        if(findButton()) {
            searchContent = document.getElementById('text1').value;
        }else {
            searchContent = document.getElementById('text2').value;
        }
        if (searchContent === ""){
            alert("请输入内容!");
        }else {
...
<div class="content">
    <!--search-->
    <div class="box" id = "search-area" style="display: block">
        <div class="title"><span>搜索</span></div>
        <form method="post" action="" name="form">
        <div class="container">
                <label><input type="radio" name="search" id="search1" value="通过标题搜索" checked>通过标题搜索：</label>
            <label>
                <input type="text" name="text1" id="text1" value="">
            </label>
            <label><input type="radio" name="search" id="search2" value="通过描述搜索">通过描述搜索：</label>
            <label>
                <textarea id="text2" name="text2" data-value=""></textarea>
            </label>
                <?php
                if (isset($_SESSION['UserName'])){
                    echo '<button onclick="showResults()"  id="button">';
                }else{
                    echo '<button onclick="judge_search()">';
                }
                ?>
                <svg xmlns="http://www.w3.org/2000/svg" class="icon" viewBox="0 0 1024 1024" width="15" height="15"
        ...
        <div class="container" id="results-container">
            ...
            function showResults(){
                if (isset($_SESSION['UserName']) && isset($_POST['submit'])){
                    if ($_POST['search'] === "通过标题搜索") {
                        echo '<script>location.href="search.php?search=通过标题搜索&content='.$_POST['text1'].'";</script>';
                    }else{
                        echo '<script>location.href="search.php?search=通过描述搜索&content='.$_POST['text2'].'";</script>';
                    }
                }else {
                    if (isset($_GET['search']) && $_GET['search'] == "通过标题搜索") {
                        echo '<script>results.style.display = \'block\';</script>';
                        searchByTitle();
                    } else if (isset($_GET['search']) && $_GET['search'] == "通过描述搜索") {
                        searchByDescription();
                        echo '<script>results.style.display = \'block\';</script>';
                    }
                }
