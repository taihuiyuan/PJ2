...
if (isset($_GET['action'])&&$_GET['action']=="reset"){
    ...
    if ($title == "" || $description == "" || $country == "" || $city == "" || $content == "") {
        ...
        $sql_title = "update travelimage set Title='$title' where PATH = '$filename'";
        $sql_description = "update travelimage set Description='$description' where PATH = '$filename'";
        $sql_content = "update travelimage set Content='$content' where PATH = '$filename'";
        $sql_city = "update travelimage set CityCode='$cityCode' where PATH = '$filename'";
        $sql_country = "update travelimage set Country_RegionCodeISO='$countryCode' where PATH = '$filename'";
        $res1 = $pdo->query($sql_title); //添加
        $res2 = $pdo->query($sql_description);
        $res3 = $pdo->query($sql_content);
        $res4 = $pdo->query($sql_city);
        $res5 = $pdo->query($sql_country);
        echo '<script>alert("修改成功!");location.href = "../photo.php";</script>';
...
<script>
    ...
    if (isset($_GET['action'])&&$_GET['action']=="revise") {
        ...
        $row1 = $result1->fetch();
        $sql2 = "select * from geocities where GeoNameID='{$row['CityCode']}'";
...
// 获取页面中的选项卡
        ...
        var selected_content="<?php echo $row['Content'];?>";
        var selected_country="<?php echo $row1['Country_RegionName'];?>";
        var selected_city="<?php echo $row2['AsciiName'];?>";
...
// 给第一个选项卡中的option赋值
        ...
        for (var i = 0; i < countries.options.length; i++) {
            ...
            if (countries.options[i].value === selected_country){
                countries.options[i].selected = true;
            }
        }

// 初始化第二个选项卡
        var country_have_selected = 0;
        for (var j = 0; j < countries.options.length; j++){
            if (countries.options[j].selected){
                country_have_selected = j;
            }
        }
        cities.options.length = city_array[country_have_selected].length;
        for (var k = 0; k < cities.options.length; k++) {
            cities.options[k].text = city_array[country_have_selected][k];
            cities.options[k].value = city_array[country_have_selected][k];
            if (cities.options[k].value === selected_city){
                cities.options[k].selected = true;
            }
        }
...
// 通过onchange监视函数，一旦第一个选项卡发生变化，第二个选项卡中的内容也跟着变化
        ...
        for (let i=0;i<contents.options.length-1;i++){
            if (contents.options[i].value === selected_content){
                contents.options[i].selected = true;
                has_selected = true;
            }
        }
        if (has_selected === false){
            contents.options[contents.options.length].selected = true;
        }
    };
</script>
...
<div class="content">
    <div class="box">
        <div class="title"><span>修改照片</span></div>
        <div class="container">

        <?php
        echo '<form action="?action=reset&id='.$ImageID.'" method="POST" name="form" enctype="multipart/form-data">';
            echo '<div class="upload-image">';
            echo '<img src="../../images/travel-images/medium/'.$row['PATH'].'" id="img3" style="width: 20vw"><br>';
            echo '</div>';

            echo '<div class="upload-content">';
            echo   '<p>图片标题</p>';
            echo '<input type="text" name="title" id="title" value="'.$row['Title'].'" required = "required">';
            echo '</div>';

            echo '<div class="upload-content">';
            echo   '<p>图片描述</p>';
            echo '<textarea id="description" name="description" required = "required">'.$row['Description'].'</textarea>';
            echo '</div>';

            echo '<div class="upload-content">';
            echo   '<p>拍摄主题</p>';
            echo '<select name="content" id="content">
                        <option value="scenery" class="content_option">Scenery</option>
                        <option value="people" class="content_option">people</option>
                        <option value="animal" class="content_option">animal</option>
                        <option value="building" class="content_option">building</option>
                        <option value="city" class="content_option">city</option>
                        <option value="wonder" class="content_option">wonder</option>
                        <option value="other" class="content_option">other</option>
                    </select>';
            echo '</div>';

            echo '<div class="upload-content">';
            echo   '<p>拍摄国家</p>';
            echo '<select name="country" id="first"></select>';
            echo '</div>';

            echo '<div class="upload-content">';
            echo   '<p>拍摄城市</p>';
            echo '<select id=\'second\' name="city"></select>';
            echo '</div>';
